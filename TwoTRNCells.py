...
def TwoTRNCells(s,t):  
    ...
    ds = np.zeros([np.max(np.size(s)),1],float) 
    v = s[0] 
    v2= s[20] 
    ...
    ds[0] =  (-1/C)*( Ina + Ik + ICa + IAR + IL)      - ix -  Isyn  - g21*(v-v2) 
    ds[20] = (-1/C)*( Ina2 + Ik2 + ICa2 + IAR2 + IL2) - ix2 - Isyn2 - g12*(v2-v) 
